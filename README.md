...
3. 読み取れた文字列が「読み取り結果」に並ぶ
   - 誤読は各文字列の `✕` で 1 件ずつ消せます（消した文字列は「候補を消す」を押すまで再検出されません）
4. 使いたい文字列をタップして「スタック」に追加
   - **全てスタックに移す** で、読み取り結果を認識した順にまとめて追加できます（スタック済みは飛ばします）
...
| `CAMERA.ZOOM` | 起動時のズーム倍率（ズーム対応端末のみ）。既定 `1`。端末でズームボタンを押して選んだ倍率が記憶されていればそちらが優先 |
| `CAMERA.ZOOM_LEVELS` | ズームボタンで順に切り替える倍率。既定 `[1, 2, 3]`。端末の上限を超える値は飛ばす。空配列でボタンを出さない |
| `CAMERA.SWITCH_BUTTON` | `true`（既定）で、背面カメラが複数ある端末に「カメラ切替」ボタンを出す。選んだカメラは端末に記憶される |
...
- **ズーム / カメラ切替ボタン**（映像の右下）: Android の一部 Galaxy はブラウザからマクロへ
  自動で切り替わらず、近づくとピントが合いません。近づかずにズームで枠を埋める、または
  背面カメラを切り替えて近接に強いレンズに当てる、の2つを対応端末でだけ出します。
  選んだ倍率とカメラはその端末に記憶されます（`localStorage`）。
  `CAMERA.ZOOM_LEVELS` を `[]`、`CAMERA.SWITCH_BUTTON` を `false` にすると出なくなります。
  `?debug=1` の診断パネルに、端末のズーム・フォーカス対応状況が表示されます。
